...
<!-- 我的待办 -->
	<head lang="en">
		...
		<script type="text/javascript">
			...
			$("#ntBugTypeSelect").val("${ntTypeSelect}");
			$("#ntBugStateSelect").val("${ntStateSelect}");
			$("#ntBugPropSelect").val("${ntPropSelect}");	
		}
		function showByFilter(){
			var ntBugTypeSelect = $("#ntBugTypeSelect").val();
			var ntBugStateSelect = $("#ntBugStateSelect").val();
			var ntBugPropSelect = $("#ntBugPropSelect").val();
			$.post("<%=path%>/filterNeedToDo",{
				"type":ntBugTypeSelect,
				"state":ntBugStateSelect,
				"prop":ntBugPropSelect,
				"bid":"${id}",
				"pid":"${pid}",
				"currpage":"1"
				},function(){	
					window.location.href="<%=path%>/common/bugAssignToMe";
				});
		} 
	...
	<body class="apps done fixed-sidebar modal-open-v">
			...
			<div class="wrapper" id="wrapper">
				...
				<nav class="navbar-default navbar-static-side">
					...
					<ul class="nav side-menu" id="side-menu">
								...
								<li class="divider"></li>
								<li data-url="bugAssignToMe.html" for="main" alt="我的待办" title="我的待办" class="li-bugAssignToMe active" id="bugAssignToMeMenu">
				...
				<div class="bug-content">
					<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
						<div class="bug-main" id="main" style="overflow: hidden; width: auto; height: 100%;">
							<div class="bugAssignToMe animated fadeInRight">
								<h4>我的待办</h4>
									...
									<ul class="nav nav-pills">
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">BUG类型：
												<select class="select"id="ntBugTypeSelect" onchange="showByFilter()">
													<option data-value="全部">全部</option>
													<option data-value="缺陷">缺陷</option>
													<option data-value="任务">任务</option>
													<option data-value="需求">需求</option>
												</select>
											</a>
										</li>
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">状态：
												<select class="select" id="ntBugStateSelect" onchange="showByFilter()">
													<option data-value="全部活动">全部活动</option>
													<option data-value="未解决">未解决</option>
													<option data-value="待审核">待审核</option>
													<option data-value="已拒绝">已拒绝</option>
												</select>
											</a>
										</li>
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">优先级：
												<select  class="select" id="ntBugPropSelect" onchange="showByFilter()">
													<option data-value="全部">全部</option>
													<option data-value="急">急</option>
													...
													<table class="table table-hover bugTable dataTable no-footer" id="dataTables">
														<thead>
															<tr role="row">
																...
																<th class="sorting_disabled" rowspan="1" colspan="1" aria-label="标题 共1条记录">标题 <span id="allCount">共${numNeed}条记录</span></th>
																...
																<th style="width:85px;" class="center sorting" tabindex="0" aria-controls="dataTables" rowspan="1" colspan="1" aria-label="计划日期：激活排序列升序">计划日期</th>
																<th style="width:85px;" class="center sorting" tabindex="0" aria-controls="dataTables" rowspan="1" colspan="1" aria-label="截止日期：激活排序列升序">截止日期</th>
																<th style="width:60px" class="sorting_disabled" rowspan="1" colspan="1" aria-label="修改人">修改人</th>
														...
														<tbody>
														<c:forEach  items="${listNeedToDo}" var="needto" varStatus="status">
															<tr class="odd"  onclick="tobug(${needto.bug_id});">
																<td class=" "><span class="fa"></span></td>
																<td class=" ">${status.index+1 }</td>
																<td class=" "><span class="label" style="padding:.3em .6em .3em;color:#fff;background:#d9534f">${needto.bug_type}</span></td>
																<td class=" "><span class="defaultColor" alt="产品" title="产品"> </span> ${needto.bug_title } </td>
																<td class=" "><span class="label label-danger">${needto.bug_state }</span></td>
																<td class=" "><span class="label label-primary">${needto.bug_prop }</span></td>
																<td class=" ">${fn:substring(needto.bug_plan_date,0,10) }</td>
																<td class=" ">${fn:substring(needto.bug_lasted_date,0,10) }</td>
																<td class=" ">${needto.bug_update}</td>
																<td class="center sorting_1">${fn:substring(needto.bug_repair_date,0,16) }</td>
															</tr>
															</c:forEach>
														</tbody>
													</table>
		...
		<li>共有${numNeed}条记录，当前第${currpage}页/共${pagecount}页</li>
	</ul>				
									</div>
								</div>
							</div>
	...
	<!--  										<div id="bugSubmitVersionDiv">
		...
		-->											
					...
					<!-- bug详情 -->
					<div class="bugInfoPage animated fadeInRightBig" id="bugInfoPage" style="display: none;">
						<div class="bugInfoTop">
							<em class="fa fa-angle-right bugInfoBack" alt="隐藏" title="隐藏"></em>
							<span class="btn0"><button class="btn btn-primary" type="button" data-action="Comment">评论</button> <button class="btn btn-primary" type="button" data-action="Assign">指派</button> <button class="btn btn-primary" type="button" data-action="Fix">修复</button> <button class="btn btn-primary" type="button" data-action="Reject">拒绝</button> <button class="btn btn-primary" type="button" data-action="Close">关闭</button> <button class="btn btn-primary" type="button" data-action="Postpone">延期</button> </span>
							<!--<span class="btn1 right"><i class="fa fa-edit" data-action="Modify" alt="修改" title="修改"></i> <i class="fa fa-copy" data-action="Copy" alt="复制/移动" title="复制/移动"></i> <i class="fa fa-trash-o" data-action="Delete" alt="删除" title="删除"></i> </span>-->
						</div>
							...
							<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
								<div class="bugInfoPageNr" style="overflow: hidden; width: auto; height: 100%;">
									<div class="panel-default bugInfo">
										<div class="row">
											<label class="col-sm-12 control-label bugInfoTitle">#2 <span class="label" style="padding:.3em .6em .3em;color:#fff;background:#d9534f">缺陷</span> 啊啊啊啊啊</label>
											<!--<div class="col-sm-12 bugDescFirstHistory">
											<div class="alert bugInfoHistories" role="alert">
												<p><span class="name"><img src="img/null.png" style="background:url(img/head_default.png) center no-repeat;background-size: cover;"><b>呜呜</b></span> 审核未通过，指派给 <span class="name"><b>呜呜</b></span> <i class="fa fa-edit hoption" alt="修改备注" title="修改备注" data-hoption="modify" data-hid="548374" data-content=""><span style="display:none"></span></i> <i class="fa fa-trash-o hoption" alt="删除备注" title="删除备注" data-hoption="delete" data-hid="548374"></i> <span class="right">2017-03-18 10:54</span></p>
											</div>
										</div>-->
										</div>
										<div class="row">
										...
										<div class="row" id="bugInfoModule" style="display: none;">
										...
										<div class="row">
											<div class="col-sm-6">
												...
												<div class="col-sm-9 bugInfoState txt pd0"><span class="label label-danger">未解决</span></div>
											</div>
											<div class="col-sm-6">
												<label class="col-sm-3 control-label pd0">优先级</label>
												<div class="col-sm-9 bugInfoPriority txt pd0"><span class="label label-primary">中</span></div>
											</div>
										</div>
										<div class="row">
											<div class="col-sm-6">
										...
										<!--<div class="row log">
											<p class="title">日志 <span class="defaultColor bugHistoryHideModifyBtn">隐藏修改记录</span></p>
					...
					<!-- 重新打开bug -->
					<div class="modal fade myProjectBugModal" id="BugReopenModal" tabindex="-4" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
									<button type="button" class="close modalExpand"><span><i class="fa fa-expand"></i></span></button>
								...
								<div class="modal-body">
									<form class="form-horizontal" id="bugReopenModelForm">
										<div class="form-group" id="reopenVersion">
											...
											<div class="col-sm-10 relative bvs">
												<div class="inputVersion">
													<input type="text" class="form-control" placeholder="最多输入50个字符" name="version" onfocus="checkStrNum(this,50)" oninput="checkStrNum(this,50)" onpropertychange="checkStrNum(this,50)">
												</div>
												<div class="customDropkick" style="width: 100%">
													<select style="visibility:hidden" tabindex="2" id="reOpenBugVersionSelect"></select>
												</div>
											</div>
										</div>
										<div class="form-group relative">
											...
											<div class="col-sm-10 relative">
												<textarea class="form-control textarea comments" placeholder="最多输入500个字符" rows="4" name="comments" onfocus="checkStrNum(this,500)" oninput="checkStrNum(this,500)" onpropertychange="checkStrNum(this,500)"></textarea>
					...
					<!-- 指派bug -->
					<div class="modal fade myProjectBugModal" id="BugAssignModal" tabindex="-5" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
										...
										<div class="form-group">
											...
											<div class="col-sm-10">
												<div class="customDropkick" id="bugAssignTo" data-uid="0">
													<div class="dk_container  dk_shown dk_theme_default" id="dk_container_bugAssignToSelect" tabindex="4">
														<a class="dk_toggle"><span class="dk_label">[未指定]</span><span class="select-icon"><i class="fa fa-caret-down"></i></span></a>
														<div class="dk_options">
															<div class="userNameSearch"><input type="text" class="form-control" placeholder="快速搜索"><i class="fa fa-times-circle"></i></div>
															<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
																<ul class="dk_options_inner" style="overflow: hidden; width: auto; height: 100%;">
																	<li class="dk_option_current">
																		<a data-dk-dropdown-value="0">[未指定]</a>
																	</li>
																	<li class="">
																		<a data-dk-dropdown-value="46604">呜呜</a>
																	</li>
																</ul>
																<div class="slimScrollBar" style="width: 7px; position: absolute; top: 0px; opacity: 0.4; display: none; border-radius: 7px; z-index: 99; right: 1px; background: rgb(0, 0, 0);"></div>
																<div class="slimScrollRail" style="width: 7px; height: 100%; position: absolute; top: 0px; display: none; border-radius: 7px; opacity: 0.9; z-index: 90; right: 1px; background: rgb(51, 51, 51);"></div>
													...
													<select tabindex="4" id="bugAssignToSelect" style="display: none;">
														<option value="0" selected="">[未指定]</option>
														<option value="46604">呜呜</option>
													</select>
												</div>
					...
					<!-- 审核不通过 -->
					<div class="modal fade myProjectBugModal" id="BugDisapproveModal" tabindex="-6" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
								...
								<div class="modal-body">
									<form class="form-horizontal" id="bugDisapproveModelForm">
										<div class="form-group" id="bugDisapproveVersion">
											...
											<div class="col-sm-10 relative bvs">
												<div class="inputVersion">
													<input type="text" class="form-control" placeholder="最多输入50个字符" name="verifyVersion" onfocus="checkStrNum(this,50)" oninput="checkStrNum(this,50)" onpropertychange="checkStrNum(this,50)">
												</div>
												<div class="customDropkick" style="width: 100%">
													<select style="visibility:hidden" tabindex="2" id="DisapproveBugVersionSelect"></select>
												</div>
											</div>
										</div>
										<div class="form-group relative">
					...
					<!-- 审核通过 -->
					<div class="modal fade myProjectBugModal" id="BugApproveModal" tabindex="-7" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
									...
									<form class="form-horizontal" id="bugApproveModelForm">
										<div class="form-group">
											...
											<div class="col-sm-10 relative bvs">
												...
												<div class="customDropkick" style="width: 100%">
													<select style="visibility:hidden" tabindex="2" id="ApproveBugVersionSelect"></select>
												</div>
											</div>
										</div>
										<div class="form-group relative">
					...
					<!-- 决绝bug -->
					<div class="modal fade myProjectBugModal" id="BugRejectModal" tabindex="-8" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
					...
					<!-- 备注bug -->
					<div class="modal fade myProjectBugModal" id="BugCommentModal" tabindex="-9" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
									<button type="button" class="close modalExpand"><span><i class="fa fa-expand"></i></span></button>
									<h4><i class="fa fa-cube"></i> 备注</h4>
								</div>
								<div class="modal-body">
					...
					<!-- 编辑bug评论 -->
					<div class="modal fade myProjectBugModal" id="BugCommentEditModal" tabindex="-9" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
					...
					<!-- 显示项目信息model -->
					<div class="modal fade myProjectModel" id="myProjectInfoModal" tabindex="-2" role="dialog" aria-labelledby="myProjectInfoBg">
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<ul class="nav nav-tabs noborder" role="tablist">
										<li role="presentation" class="active" style="width:33.333333333%">
											<a href="bugActive.html#projectShowInfo" aria-controls="projectShowInfo" role="tab" data-toggle="tab">项目信息</a>
										</li>
										<li role="presentation" style="width:33.333333333%">
											<a href="bugActive.html#projectShowUser" aria-controls="projectShowUser" role="tab" data-toggle="tab">成员列表</a>
										</li>
										<li role="presentation" style="width:33.333333333%">
											<a href="bugActive.html#projectShowProduct" aria-controls="projectShowProduct" role="tab" data-toggle="tab">产品列表</a>
										</li>
									</ul>
									<!-- Tab panes -->
									<div class="tab-content">
										<div role="tabpanel" class="tab-pane active" id="projectShowInfo">
											<div class="row">
												<form class="form-horizontal" id="showProjectForm" name="showProjectForm">
													<div class="form-group">
														<div class="col-sm-3 text-right"><b>项目名称</b></div>
														<div class="col-sm-9 pName"></div>
													</div>
													<div class="form-group">
														<div class="col-sm-3 text-right"><b>项目描述</b></div>
														<div class="col-sm-9 pDesc"></div>
													</div>
													<div class="form-group">
														<div class="col-sm-3 text-right"><b>拥有人</b></div>
														<div class="col-sm-9 pUser"></div>
													</div>
													<div class="form-group">
														<div class="col-sm-3 text-right"><b>创建时间</b></div>
														<div class="col-sm-9 pDate"></div>
													</div>
												</form>
											</div>
										</div>
										<div role="tabpanel" class="tab-pane projectUser" id="projectShowUser">
											<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
												<div class="projectUserList md34" id="projectUserList" style="overflow: hidden; width: auto; height: 100%;">
										...
										<div role="tabpanel" class="tab-pane projectProject" id="projectShowProduct">
											<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
												<div class="projectProductList md34" id="projectProductList" style="overflow: hidden; width: auto; height: 100%;">
													<ul class="list-group"></ul>
												</div>
												<div class="slimScrollBar" style="width: 7px; position: absolute; top: 0px; opacity: 0.4; display: none; border-radius: 7px; z-index: 99; right: 1px; background: rgb(0, 0, 0);"></div>
												<div class="slimScrollRail" style="width: 7px; height: 100%; position: absolute; top: 0px; display: none; border-radius: 7px; opacity: 0.9; z-index: 90; right: 1px; background: rgb(51, 51, 51);"></div>
					...
					<!-- 发布通知 -->
					<div class="modal fade myProjectBugModal" id="myProjectNoticeModal" tabindex="-12" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
								...
								<div class="modal-body">
									<form class="form-horizontal" id="noticeModelForm" name="noticeModelForm">
										<div class="form-group">
											<label class="col-sm-2 control-label">类型</label>
											<div class="col-sm-10">
												<div class="customDropkick" style="width: 100%">
													<div class="dk_container  dk_shown dk_theme_default" id="dk_container_noticeType" tabindex="4">
														<a class="dk_toggle"><span class="dk_label">信息公布</span><span class="select-icon"><i class="fa fa-caret-down"></i></span></a>
														<div class="dk_options">
															<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
																<ul class="dk_options_inner" style="overflow: hidden; width: auto; height: 100%;">
																	<li class="dk_option_current">
																		<a data-dk-dropdown-value="Information">信息公布</a>
																	</li>
																	<li class="">
																		<a data-dk-dropdown-value="MeetingMinutes">会议记要</a>
																	</li>
																	<li class="">
																		<a data-dk-dropdown-value="VersionRelease">版本发布</a>
																	</li>
																	<li class="">
																		<a data-dk-dropdown-value="MeetingNotice">会议通知</a>
																	</li>
																	<li class="">
																		<a data-dk-dropdown-value="WorkPlan">工作计划</a>
																	</li>
																</ul>
																<div class="slimScrollBar" style="width: 7px; position: absolute; top: 0px; opacity: 0.4; display: none; border-radius: 7px; z-index: 99; right: 1px; background: rgb(0, 0, 0);"></div>
																<div class="slimScrollRail" style="width: 7px; height: 100%; position: absolute; top: 0px; display: none; border-radius: 7px; opacity: 0.9; z-index: 90; right: 1px; background: rgb(51, 51, 51);"></div>
													...
													<select style="visibility: hidden; display: none;" tabindex="4" id="noticeType">
										...
										<div class="form-group">
											...
											<div class="col-sm-10 relative">
												<textarea class="form-control textarea" rows="6" name="description" placeholder="最多输入2000个字符" onfocus="checkStrNum(this,2000)" oninput="checkStrNum(this,2000)" onpropertychange="checkStrNum(this,2000)"></textarea>
					...
					<!-- 修改通知 -->
					<div class="modal fade myProjectBugModal" id="myProjectNoticeEditModal" tabindex="-12" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
								...
								<div class="modal-body">
									<form class="form-horizontal" id="noticeEditModelForm" name="noticeEditModelForm">
										<div class="form-group">
											<label class="col-sm-2 control-label">类型</label>

											<div class="col-sm-10">
												<div class="customDropkick" style="width: 100%">
													<select style="visibility:hidden" tabindex="4" id="editNoticeType">
													</select>
												</div>
											</div>
										</div>
					...
					<!-- 通知详情 -->
					<div class="bugInfoPage animated fadeOutRightBig" id="noticeInfoPage">
						<div class="noticeInfoTop">
							<em class="fa fa-angle-right noticeInfoBack" alt="隐藏" title="隐藏"></em>
							<span class="comment"></span>
							<span class="top"></span>
							<span class="right dele"></span>
							<span class="right options"></span>
						</div>
						<div class="bugInfoContent">
							<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
								<div class="bugInfoPageNr" style="overflow: hidden; width: auto; height: 100%;">
									<div class="panel-default noticeInfo">
										<div class="row">
											<label class="col-sm-12 control-label noticeInfoTitle"></label>

											<p class="noticeUserTime" id="noticeUserTime"></p>
										</div>
										<div class="row">
											<div class="col-sm-12 noticeInfoDescription"></div>
										</div>
										<div class="row" id="noticeInfoImages">
											<div class="col-sm-12 noticeInfoImages"></div>
										</div>
										<div class="row">
											<div class="col-sm-12 noticeInfoFujian"></div>
										</div>
										<div class="row log">
											<p class="title">评论</p>
						...
						<div id="noticeInfoDrag" class="noticeInfoDrag"></div>
					</div>

					<!-- 评论通知 -->
					<div class="modal fade myProjectBugModal" id="NoticeCommentModal" tabindex="-13" role="dialog" data-backdrop="static" data-keyboard="false">
						<div class="modal-dialog" role="document" style="z-index: 1040">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><i class="fa fa-remove"></i></span></button>
								...
								<div class="modal-body">
									<form class="form-horizontal" id="NoticeCommentModelForm">
										<div class="form-group">
