...
<!--我的任务 -->
	<head lang="en">
		...
		window.onload = function(){
			$("#mtBugTypeSelect").val("${mtTypeSelect}");
			$("#mtBugStateSelect").val("${mtStateSelect}");
			$("#mtBugPropSelect").val("${mtPropSelect}");
			$("#mtBugSignerSelect").val("${mtSignerSelect}");		
			var f = ${myAllNameSize};
			var ff = ${mtSignerSelect};
			if(f==0 || !ff.equals("全部")){
		...
		function showByFilter(){
			var mtBugTypeSelect = $("#mtBugTypeSelect").val();
			var mtBugStateSelect = $("#mtBugStateSelect").val();
			var mtBugPropSelect = $("#mtBugPropSelect").val();
			var mtBugSignerSelect = $("#mtBugSignerSelect").val();
			$.post("<%=path%>/filterMyAllTask",{
				"type":mtBugTypeSelect,
				"state":mtBugStateSelect,
				"prop":mtBugPropSelect,
				"signer":mtBugSignerSelect,
				"bid":"${id}",
				"pid":"${pid}",
				"currpage":"1"
				},function(){	
					window.location.href="<%=path%>/common/bugFixByMe";
				});
		} 
...
function tobug(a){
	...
	<body class="apps done fixed-sidebar modal-open-v">
					...
					<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
						<div class="sidebar-collapse" style="overflow: hidden; width: auto; height: 100%;">
							...
							<ul class="nav side-menu" id="side-menu">
								...
								<li data-url="bugFixByMe.html" for="main" alt="我的任务" title="我的任务" class="li-bugFixByMe active" id="bugFixByMeMenu" style="">
				...
				<div class="bug-content">
					<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
						<div class="bug-main" id="main" style="overflow: hidden; width: auto; height: 100%;">
							<div class="bugFixByMe animated fadeInRight">
								<h4>我的任务</h4>
									...
									<ul class="nav nav-pills">
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">BUG类型：
												<select class="select" onchange="showByFilter()" id="mtBugTypeSelect">
										...
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">状态：
												<select class="select" onchange="showByFilter()" id="mtBugStateSelect">
													<option data-value="">全部</option>
										...
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">优先级：
												<select class="select" onchange="showByFilter()" id="mtBugPropSelect">
													<option data-value="">全部</option>
										...
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">分配人：
												<select class="select" onchange="showByFilter()" id="mtBugSignerSelect">
													<option data-value="">全部</option>
													<c:forEach  items="${myAllTaskSigner}" var="mtSignerList">
													<option class="oneName" data-value="${mtSignerList.bug_signer}">${mtSignerList.bug_signer}</option>
													</c:forEach>
													<option style="display:none;" class="twoName" data-value="${mtSignerSelect}">${mtSignerSelect }</option>
												</select>
											</a>
										</li>
									</ul>
													...
													<table class="table table-hover bugTable dataTable no-footer" id="dataTables">
														<thead>
															<tr role="row">
																...
																<th class="sorting_disabled"  rowspan="1" colspan="1" aria-label="标题 共2条记录">标题 <span id="allCount">共${numMyAll}条记录</span></th>
																...
																<th style="width:60px" class="sorting_disabled" rowspan="1" colspan="1" aria-label="处理人">处理人</th>
																<th style="width:60px" class="sorting_disabled" rowspan="1" colspan="1" aria-label="分配人">分配人</th>
														...
														<tbody>
															<c:forEach  items="${listMyAllTask}"  var="myalltask" varStatus="status">
															<tr class="odd"  onclick="tobug(${myalltask.bug_id});">
																<td class=" "><span class="fa"></span></td>
																<td class=" ">${status.index+1 }</td>
																<td class=" "><span class="label" style="padding:.3em .6em .3em;color:#fff;background:#d9534f">${myalltask.bug_type}</span></td>
																<td class=" "><span class="defaultColor" alt="产品" title="产品"> </span> ${myalltask.bug_title } </td>
																<td class=" "><span class="label label-danger">${myalltask.bug_state }</span></td>
																<td class=" "><span class="label label-primary">${myalltask.bug_prop }</span></td>
																<td class=" ">${myalltask.bug_maker }</td>
																<td class=" ">${myalltask.bug_signer }</td>
																<td class=" ">${fn:substring(myalltask.bug_create_date,0,16) }</td>
																<td class="center sorting_1">${fn:substring(myalltask.bug_repair_date,0,16)}</td>
															</tr>
															</c:forEach>
														</tbody>
													</table>
		...
		<li>共有${numMyAll}条记录，当前第${currpage}页/共${pagecount}页</li>
	</ul>				
									</div>
								</div>
							</div>
