...
<html class=" dk_fouc">
	...
	function bug(){
		var _from = document.getElementById("bug");
		_from.submit();
	}		
	</script>
<script type="text/javascript">
window.onload = function(){
		$("#actBugTypeSelect").val("${actTypeSelect}");
		$("#actBugStateSelect").val("${actStateSelect}");
		$("#actBugPropSelect").val("${actPropSelect}");
		$("#actBugMakerSelect").val("${actMakerSelect}");	
		var a = ${activeMakerSize};
		var b = ${actMakerSelect};
		if(a==0 || !b.equals("全部")){
			$(".oneMakerName").css("display","none");
			$(".twoMakerName").css("display","inline");
		}
		if(a==1){
			$(".twoMakerName").css("display","none");
		}
	}
	function showByFilter(){
		var actBugTypeSelect = $("#actBugTypeSelect").val();
		var actBugStateSelect = $("#actBugStateSelect").val();
		var actBugPropSelect = $("#actBugPropSelect").val();
		var actBugMakerSelect = $("#actBugMakerSelect").val();
		$.post("<%=path%>/filterActiveBug",{
			"type":actBugTypeSelect,
			"state":actBugStateSelect,
			"prop":actBugPropSelect,
			"maker":actBugMakerSelect,
			"bid":"${id}",
			"pid":"${pid}",
			"currpage":"1"
			},function(){	
				window.location.href="<%=path%>/common/bugActive";
			});
	} 
...
function FileUpload(){
	...
	<body class="apps done fixed-sidebar modal-open-v">
            		...
            		<i class="fa fa-angle-right"></i></span>
						<div class="sidebar-select" id="console-sidebar-select" alt="切换项目" title="切换项目">
							<div class="dk_container  dk_shown dk_theme_default" id="dk_container_projectSelectList" tabindex="1">
								...
								<div class="dk_options" style="top: 52px;">
					...
					<div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 100%;">
						<div class="sidebar-collapse" style="overflow: hidden; width: auto; height: 100%;">
							...
							<ul class="nav side-menu" id="side-menu">
								...
								<li class="li-bugActive active" data-url="" for="main" alt="活动BUG" title="活动BUG" id="bugActiveMenu">
									<a href="<%=path%>/Programactive?currpage=1&bid=${id}&pid=${pid}"><i class="fa fa-clock-o"></i> <span class="nav-label">活动BUG <sup class="badge bgDefault" id="bugsActive">${num}</sup></span></a><span class="right fa fa-angle-left" alt="隐藏" title="隐藏"></span>
								</li>
									...
									<ul class="nav nav-pills">
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">BUG类型：
												<select class="select" onchange="showByFilter()" id="actBugTypeSelect">
										...
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">状态：
												<select class="select"  onchange="showByFilter()" id="actBugStateSelect">
													<option data-value="Active">全部活动</option>
										...
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">优先级：
												<select class="select"onchange="showByFilter()" id="actBugPropSelect">
													<option data-value="">全部</option>
										...
										<li role="presentation" class="dropdown">
											<a class="dropdown-toggle">指派给：
												<select  class="select" onchange="showByFilter()" id="actBugMakerSelect">
													<option data-value="全部">全部</option>
													<option data-value="[未指定]">[未指定]</option>
													<c:forEach  items="${activeMakerList}" var="activeMaker">
													<option class="oneMakerName" data-value="${activeMaker.bug_toman}">${activeMaker.bug_toman}</option>
													</c:forEach>
													<option style="display:none;" class="twoMakerName" data-value="${actMakerSelect}">${actMakerSelect }</option>
													...
													<table class="table table-hover bugTable dataTable no-footer" id="dataTables">
														<thead>
															<tr role="row">
																...
																<th class="sorting_disabled" rowspan="1" colspan="1" aria-label="标题 共2条记录">标题 <span id="allCount">共${num}条记录</span></th>
																...
																<th style="width:120px" class="sorting_disabled" rowspan="1" colspan="1" aria-label="指派给">指派给</th>
														...
														<tbody>		
															...
															<tr class="odd"  onclick="tobug(${a.bug_id});">
																...
																<td class=" "><span class="label" style="padding:.3em .6em .3em;color:#fff;background:#d9534f">${a.bug_type}</span></td>
																<td class=" "><span class="defaultColor" alt="产品" title="产品"> </span>${a.bug_title} </td>
																<td class=" "><span class="label label-danger">${a.bug_state }</span></td>
																<td class=" "><span class="label label-primary">${a.bug_prop}</span></td>
																<td class=" ">${a.bug_toman}</td>
																<td class=" center">${fn:substring(a.bug_plan_date,0,10)}</td>
																<td class=" center">${fn:substring(a.bug_lasted_date,0,10)}</td>
		...
		<li>共有${num}条记录，当前第${currpage}页/共${pagecount}页</li>
	</ul>				       
</div>
											
										</div>
									</div>
									<div class="row viewCalendar" style="display: none;">
										<div id="calendar"></div>
									</div>
									<div class="row viewKanban" style="display: none;">
										<div id="kanban">
											<div class="k-header">
												<div class="inline leftDiv"></div>
												<div class="inline right rightDiv"><span class="title" onselectstart="return false">显示标题</span><span class="check"><i class="fa "></i></span></div>
											</div>
											<div class="k-table relative">
						...
						<div class="slimScrollBar" style="width: 7px; position: absolute; top: 0px; opacity: 0.4; display: none; border-radius: 7px; z-index: 99; right: 1px; height: 555px; background: rgb(0, 0, 0);"></div>
						<div class="slimScrollRail" style="width: 7px; height: 100%; position: absolute; top: 0px; display: none; border-radius: 7px; opacity: 0.9; z-index: 90; right: 1px; background: rgb(51, 51, 51);"></div>
					</div>
				</div>
				<!-- main end -->
