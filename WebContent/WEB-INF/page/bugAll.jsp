...
window.onload = function(){
			$("#allBugTypeSelect").val("${allTypeSelect}");
			$("#allBugStateSelect").val("${allStateSelect}");
			$("#allBugPropSelect").val("${allPropSelect}");
			$("#allBugMakerSelect").val("${allMakerSelect}");	
			$("#allBugSignerSelect").val("${allSignerSelect}");	
			$("#allBugCreaterSelect").val("${allCreaterSelect}");	
			var a = ${allMakerSize};
			var aa = ${allMakerSelect};
			if(a==0 || !aa.equals("全部")){
			...
			var b = ${allSignerSize};
			var bb = ${allSignerSelect};
			if(b==0 || !bb.equals("全部")){
				$(".oneSignerName").css("display","none");
				$(".twoSignerName").css("display","inline");
			}
			if(b==1){
				$(".twoSignerName").css("display","none");
			}
			var c = ${allCreaterSize};
			var cc = ${allCreaterSelect};
			if(c==0 || !cc.equals("全部")){
				$(".oneCreaterName").css("display","none");
				$(".twoCreaterName").css("display","inline");
			}
			if(c==1){
				$(".twoCreaterName").css("display","none");
			}
		}
		function showByFilter(){
			var allBugTypeSelect = $("#allBugTypeSelect").val();
			var allBugStateSelect = $("#allBugStateSelect").val();
			var allBugPropSelect = $("#allBugPropSelect").val();
			var allBugMakerSelect = $("#allBugMakerSelect").val();
			var allBugSignerSelect = $("#allBugSignerSelect").val();
			var allBugCreaterSelect = $("#allBugCreaterSelect").val();
			$.post("<%=path%>/filterAllBug",{
				"type":allBugTypeSelect,
				"state":allBugStateSelect,
				"prop":allBugPropSelect,
				"maker":allBugMakerSelect,
				"signer":allBugSignerSelect,
				"creater":allBugCreaterSelect,
				"bid":"${id}",
				"pid":"${pid}",
				"currpage":"1"
				},function(){	
...
<body class="apps done fixed-sidebar modal-open-v">
				...
				<div class="sidebar-collapse"
					...
					<ul class="nav side-menu" id="side-menu">
						...
						<li data-url="" for="main" alt="所有BUG" title="所有BUG"
							class="li-bugAll active" id="bugAllMenu"><a
						...
						<li data-url="noticeAll.html" for="main" alt="项目通知" title="项目通知"
							...
							href="<%=path%>/showNotice?bid=${id }&pid=${pid}&currpage=1">
							<i class="fa fa-bullhorn"></i> <span class="nav-label">项目通知</span></a><span
							class="right fa fa-angle-left" alt="隐藏" title="隐藏"></span></li>
							...
							<ul class="nav nav-pills">
									<li role="presentation" class="dropdown">
										<a class="dropdown-toggle">BUG类型：
											<select class="select" onchange="showByFilter()" id="allBugTypeSelect">
									...
									<li role="presentation" class="dropdown">
										<a class="dropdown-toggle">状态：
											<select class="select" onchange="showByFilter()" id="allBugStateSelect">
									...
									<li role="presentation" class="dropdown">
										<a class="dropdown-toggle">优先级：
											<select class="select" onchange="showByFilter()" id="allBugPropSelect">
												<option data-value="">全部</option>
									...
									<li role="presentation" class="dropdown">
										<a class="dropdown-toggle">指派给：
											<select class="select" onchange="showByFilter()" id="allBugMakerSelect">
												<option data-value="全部">全部</option>
												<option data-value="[未指定]">[未指定]</option>
												<c:forEach  items="${allMakerList}" var="allMaker">
													<option class="oneMakerName" data-value="${allMaker.bug_toman}">${allMaker.bug_toman}</option>
													</c:forEach>
													<option style="display:none;" class="twoMakerName" data-value="${allMakerSelect}">${allMakerSelect }</option>
											</select>
										</a>
									</li>
									<li role="presentation" class="dropdown">
										<a class="dropdown-toggle">分配人：
											<select class="select" onchange="showByFilter()" id="allBugSignerSelect">
												<option data-value="">全部</option>
												<c:forEach  items="${allSignerList}" var="allSigner">
													<option class="oneSignerName" data-value="${allSigner.bug_signer}">${allSigner.bug_signer}</option>
													</c:forEach>
													<option style="display:none;" class="twoSignerName" data-value="${allSignerSelect}">${allSignerSelect }</option>
											</select>
										</a>
									</li>
									<li role="presentation" class="dropdown">
										<a class="dropdown-toggle">创建人：
											<select class="select"onchange="showByFilter()" id="allBugCreaterSelect">
												<option data-value="">全部</option>
												<c:forEach  items="${allCreaterList}" var="allCreater">
													<option class="oneCreaterName" data-value="${allCreater.bug_creator}">${allCreater.bug_creator}</option>
													</c:forEach>
													<option style="display:none;" class="twoCreaterName" data-value="${allCreaterSelect}">${allCreaterSelect}</option>
											</select>
										</a>
									</li>
								</ul>
											...
											<table class="table table-hover bugTable dataTable no-footer"
												...
												<thead>
													<tr role="row">
														...
														<th style="width: 85px" class="sorting_disabled"
	...
	-->
		...
		<!--  -->
		<script type="text/javascript"
			src="<%=path %>/css/bootstrap-3.3.7-dist/js/bootstrap.min.js"></script>
</body>
